...
More explicity, we are taking the average over the set of shapes $\boldsymbol{X}_1, ..., \boldsymbol{X}_n$, where $\boldsymbol{X}_i = \left(x^{(i)}_1, x^{(i)}_2, ... x^{(i)}_k\right)$ and $x^{(i)}_j = \begin{bmatrix}{c_1\\c_2\\\vdots\\c_d}\end{bmatrix}$.
