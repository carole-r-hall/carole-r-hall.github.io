...
I already mentioned that the pre-shape space $S^{kd-d-1}$ is a unit sphere (remember: this is the space of configurations that are invariant to translation and scaling but _not_ rotation). Even though we have our shape space $\Sigma_k^d$, the use of the pre-shape sphere can be very convenient&mdash;speficially, if we imagine the tangent space $\boldsymbol{T}_X$ of our pre-shape sphere near the point (or configuration) $\boldsymbol{X}$, then we can linearly approximate our pre-shape sphere, allowing us to do nice things that rely on a space being linear (like principal component analysis, a.k.a. PCA, for example). Additionally, linear distances on $\boldsymbol{T}$ closely resemble distances on the pre-shape sphere, so long as we remain close to the reference point $\boldsymbol{X}$. When we work on a tangent space like $\boldsymbol{T}_{\boldsymbol{X}}$, we can map between the tangent space and the original space easily by using the logarithmic and exponential maps; the **logarithmic** map gives us a way to map from our original space to the tangent space. Letting $\boldsymbol{X}, \boldsymbol{Y} \in \boldsymbol{S}^{kd - d - 1}$ (remembering that points in this pre-shape space are configurations invariant to scaling and translation): 
